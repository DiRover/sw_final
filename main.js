...
var url = 'https://service-worker-ahj.herokuapp.com/data'; //это нужно, чтобы апп.дж не работал в контексте сервис воркера
...
var Store = /*#__PURE__*/function () {
  ...
  _createClass(Store, [{
    ...
    key: "getNetworkData",
    value: function getNetworkData(status) {
      //метод для получения и обработки статуса запроса, из сети он или из кэша
...
function RenderWidget_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function RenderWidget_defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function RenderWidget_createClass(Constructor, protoProps, staticProps) { if (protoProps) RenderWidget_defineProperties(Constructor.prototype, protoProps); if (staticProps) RenderWidget_defineProperties(Constructor, staticProps); return Constructor; }




var RenderWidget = /*#__PURE__*/function () {
  // класс для списка проектов
  function RenderWidget(store, container) {
    RenderWidget_classCallCheck(this, RenderWidget);
  ...
  RenderWidget_createClass(RenderWidget, [{
    key: "init",
    value: function init() {
      var _this = this;
      ...
      .pipe(pluck('info') // оператором pluck ловим в нём объект с проектами по ключу info
    ...
    value: function render(info) {
      // метод для рендера страницы
      var data = info.data; //методом деструктуризации получаем объекты с нужной информацией

      var status = info.status; //статус показывает от куда получили ответ (сеть или кэш), также это css класс по которому отображается или нет сообщение от том, что нет связи
      ...
      var widget = document.createElement('div'); //создаём всякие нужные элементы
      ...
      msg.innerHTML = "<div class = '".concat(status, "'>Error: failed to connect, server not responding. You are  seeing data from cache.</div>");
      ...
      data.forEach(function (i) {
        ...
        var elem = "<img src= data:image/svg+xml;base64,".concat(btoa(avatar), " alt='avatar'/>");
        filmBox.innerHTML = "\n        <div class = 'film-name'>".concat(i.name, "</div>\n        <div class = 'film-body'>\n            ").concat(elem, "\n            <div class = 'film-text'>\n                <div class = 'text'><span class = 'genre'>Genre:</span> ").concat(i.genre, "</div>\n                <div class = 'text'><span class = 'description'>description:</span> ").concat(i.description, "</div>\n            </div>\n        </div>\n        ");
...
function handler(target, store, url) {
  // обработчик кликов получает цель клика и экземпляр класса Store
  if (target.classList.contains('btn-refresh')) {
    //должны попасть по кнопке
...
var container = document.querySelector('.container'); //получаем основной контейнер для отрисовки приложения

var store = new Store(); //класс для создания и управления потоками RxJS

var renderWidget = new RenderWidget(store, container); //класс для отрисовки страницы потомками

if (navigator.serviceWorker) {
  //проверяем наличие в браузере сервис воркера
  ...
  navigator.serviceWorker.addEventListener('message', function (evt) {
    //получаем сообщения от сервис воркера
    console.log('message!!');
